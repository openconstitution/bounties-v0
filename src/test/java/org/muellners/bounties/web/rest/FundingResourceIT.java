...
public class FundingResourceIT {
   ...
   *
   * @see
   *     org.muellners.bounties.repository.search.FundingSearchRepositoryMockConfiguration
   */
  @Autowired private FundingSearchRepository mockFundingSearchRepository;

  @Autowired private EntityManager em;

  @Autowired private MockMvc restFundingMockMvc;
  ...
  public void getAllFundings() throws Exception {
    ...
    restFundingMockMvc.perform(get("/api/fundings?sort=id,desc"))
        ...
        .andExpect(
            jsonPath("$.[*].id").value(hasItem(funding.getId().intValue())))
        .andExpect(
            jsonPath("$.[*].amount").value(hasItem(DEFAULT_AMOUNT.intValue())))
        .andExpect(jsonPath("$.[*].mode").value(hasItem(DEFAULT_MODE)))
        .andExpect(jsonPath("$.[*].paymentAuth")
                       .value(hasItem(DEFAULT_PAYMENT_AUTH.booleanValue())));
  }
  ...
  public void getFunding() throws Exception {
    ...
    restFundingMockMvc.perform(get("/api/fundings/{id}", funding.getId()))
        ...
        .andExpect(jsonPath("$.paymentAuth")
                       .value(DEFAULT_PAYMENT_AUTH.booleanValue()));
  }
  @Test
  @Transactional
  ...
  public void updateFunding() throws Exception {
    ...
    // Disconnect from session so that the updates on updatedFunding are not
    // directly saved in db
    em.detach(updatedFunding);
    updatedFunding.amount(UPDATED_AMOUNT)
        .mode(UPDATED_MODE)
        .paymentAuth(UPDATED_PAYMENT_AUTH);
  ...
  public void deleteFunding() throws Exception {
    ...
    // Delete the funding
    restFundingMockMvc
        .perform(delete("/api/fundings/{id}", funding.getId())
                     .with(csrf())
  ...
  public void searchFunding() throws Exception {
    ...
    // Initialize the database
    fundingRepository.save(funding);
    when(mockFundingSearchRepository.search(
             queryStringQuery("id:" + funding.getId())))
        .thenReturn(Collections.singletonList(funding));

    // Search the funding
    restFundingMockMvc
        .perform(get("/api/_search/fundings?query=id:" + funding.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
        .andExpect(
            jsonPath("$.[*].id").value(hasItem(funding.getId().intValue())))
