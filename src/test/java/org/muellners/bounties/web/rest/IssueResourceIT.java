...
public class IssueResourceIT {
   ...
   *
   * @see
   *     org.muellners.bounties.repository.search.IssueSearchRepositoryMockConfiguration
   */
  @Autowired private IssueSearchRepository mockIssueSearchRepository;

  @Autowired private EntityManager em;

  @Autowired private MockMvc restIssueMockMvc;
  ...
  public void getAllIssues() throws Exception {
    ...
    restIssueMockMvc.perform(get("/api/issues?sort=id,desc"))
        ...
        .andExpect(
            jsonPath("$.[*].id").value(hasItem(issue.getId().intValue())))
  ...
  public void updateIssue() throws Exception {
    ...
    // Disconnect from session so that the updates on updatedIssue are not
    // directly saved in db
    em.detach(updatedIssue);
    updatedIssue.issueId(UPDATED_ISSUE_ID)
        .url(UPDATED_URL)
        .description(UPDATED_DESCRIPTION);

    restIssueMockMvc
        .perform(put("/api/issues")
                     .with(csrf())
                     .contentType(MediaType.APPLICATION_JSON)
  ...
  public void deleteIssue() throws Exception {
    ...
    // Delete the issue
    restIssueMockMvc
        .perform(delete("/api/issues/{id}", issue.getId())
                     .with(csrf())
  ...
  public void searchIssue() throws Exception {
    ...
    // Initialize the database
    issueRepository.saveAndFlush(issue);
    when(mockIssueSearchRepository.search(
             queryStringQuery("id:" + issue.getId())))
        .thenReturn(Collections.singletonList(issue));

    // Search the issue
    restIssueMockMvc
        .perform(get("/api/_search/issues?query=id:" + issue.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
        .andExpect(
            jsonPath("$.[*].id").value(hasItem(issue.getId().intValue())))
