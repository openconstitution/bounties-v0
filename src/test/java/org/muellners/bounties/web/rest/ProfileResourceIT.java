...
<<<<<<< HEAD
   ...
   *
   * @see
   *     org.muellners.bounties.repository.search.ProfileSearchRepositoryMockConfiguration
   */
  @Autowired private ProfileSearchRepository mockProfileSearchRepository;

  @Autowired private EntityManager em;

  @Autowired private MockMvc restProfileMockMvc;
  ...
  public void createProfile() throws Exception {
    ...
    assertThat(testProfile.getGithubOrgName())
        .isEqualTo(DEFAULT_GITHUB_ORG_NAME);
  ...
  public void getAllProfiles() throws Exception {
    ...
    restProfileMockMvc.perform(get("/api/profiles?sort=id,desc"))
        ...
        .andExpect(
            jsonPath("$.[*].id").value(hasItem(profile.getId().intValue())))
        .andExpect(jsonPath("$.[*].votes").value(hasItem(DEFAULT_VOTES)))
        .andExpect(
            jsonPath("$.[*].profilelink").value(hasItem(DEFAULT_PROFILELINK)))
        .andExpect(jsonPath("$.[*].about").value(hasItem(DEFAULT_ABOUT)))
        .andExpect(jsonPath("$.[*].walletaddress")
                       .value(hasItem(DEFAULT_WALLETADDRESS)))
        .andExpect(
            jsonPath("$.[*].githubEmail").value(hasItem(DEFAULT_GITHUB_EMAIL)))
        .andExpect(jsonPath("$.[*].githubOrgName")
                       .value(hasItem(DEFAULT_GITHUB_ORG_NAME)));
  }
  ...
  public void updateProfile() throws Exception {
    ...
    // Disconnect from session so that the updates on updatedProfile are not
    // directly saved in db
    em.detach(updatedProfile);
    updatedProfile.votes(UPDATED_VOTES)
        .profilelink(UPDATED_PROFILELINK)
        .about(UPDATED_ABOUT)
        .walletaddress(UPDATED_WALLETADDRESS)
        .githubEmail(UPDATED_GITHUB_EMAIL)
    ...
    assertThat(testProfile.getGithubOrgName())
        .isEqualTo(UPDATED_GITHUB_ORG_NAME);
  ...
  public void deleteProfile() throws Exception {
    ...
    // Delete the profile
    restProfileMockMvc
        .perform(delete("/api/profiles/{id}", profile.getId())
                     .with(csrf())
  ...
  public void searchProfile() throws Exception {
    ...
    // Initialize the database
    profileRepository.saveAndFlush(profile);
    when(mockProfileSearchRepository.search(
             queryStringQuery("id:" + profile.getId())))
        .thenReturn(Collections.singletonList(profile));

    // Search the profile
    restProfileMockMvc
        .perform(get("/api/_search/profiles?query=id:" + profile.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
        .andExpect(
            jsonPath("$.[*].id").value(hasItem(profile.getId().intValue())))
