...
public class BountiesResource {
  ...
  public ResponseEntity<Bounties> createBounties(@RequestBody Bounties bounties)
    ...
    if (bounties.getId() != null) {
      throw new BadRequestAlertException(
          "A new bounties cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     body the updated bounties,
   * or with status {@code 400 (Bad Request)} if the bounties is not valid,
   * or with status {@code 500 (Internal Server Error)} if the bounties couldn't
   * be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/bounties")
  public ResponseEntity<Bounties> updateBounties(@RequestBody Bounties bounties)
      throws URISyntaxException {
    log.debug("REST request to update Bounties : {}", bounties);
    if (bounties.getId() == null) {
      throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
    }
    if (!bounties.getCreatedBy().equals(
            SecurityUtils.getCurrentUserLoginString())) {
      throw new BadRequestAlertException("Operation not permitted", ENTITY_NAME,
                                         "Operation not allowed");
    }
    Bounties result = bountiesService.save(bounties);
    return ResponseEntity.ok()
        .headers(HeaderUtil.createEntityUpdateAlert(
            applicationName, true, ENTITY_NAME, bounties.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with
   *     body the bounties, or with status {@code 404 (Not Found)}.
   */
  ...
  /**
   * {@code SEARCH  /_search/bounties?query=:query} : search for the bounties
