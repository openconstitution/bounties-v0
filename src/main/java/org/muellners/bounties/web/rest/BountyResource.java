...
public class BountyResource {
  ...
  @PreAuthorize("hasAuthority(\"" + AuthoritiesConstants.USER + "\")")
  public ResponseEntity<BountyDTO>
  removeFunding(@PathVariable("id") final Long id,
                @PathVariable("fundingId") final Long fundingId)
      throws URISyntaxException {
    log.debug("REST request to remove funding from Bounty : {}", id);

    final BountyDTO bountyDTO = bountyService.findOne(id);
    final FundingDTO fundingDTO =
        (FundingDTO)bountyDTO.getFundings().stream().map(fundingDto -> {
