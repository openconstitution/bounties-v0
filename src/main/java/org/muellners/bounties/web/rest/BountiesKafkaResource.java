...
public class BountiesKafkaResource {
  ...
  private final Logger log =
      LoggerFactory.getLogger(BountiesKafkaResource.class);
  ...
  public PublishResult publish(@PathVariable String topic,
                               @RequestParam String message,
                               @RequestParam(required = false) String key)
      throws ExecutionException, InterruptedException {
    log.debug(
        "REST request to send to Kafka topic {} with key {} the message : {}",
        topic, key, message);
  ...
  private static class PublishResult {
    ...
    private PublishResult(String topic, int partition, long offset,
                          Instant timestamp) {
      this.topic = topic;
