...
public class BountyMapper {
  ...
  public BountyDTO bountyToBountyDTO(final Bounty bounty) {
    ...
    bountyDTO.setFundings(bounty.getFundings()
                              .stream()
                              .map(funding -> {
                                return fundingMapper.fundingToFundingDTO(
                                    funding);
                              })
  ...
  public Bounty bountyDTOToBounty(final BountyDTO bountyDTO) {
    if (bountyDTO == null) {
      ...
      bounty.setFundings(bountyDTO.getFundings()
                             .stream()
                             .map(fundingDTO -> {
                               return fundingMapper.fundingDTOToFunding(
                                   fundingDTO);
                             })
                             .collect(Collectors.toSet()));
      return bounty;
