...
export const ACTION_TYPES = {
  SEARCH_FUNDINGS: 'funding/SEARCH_FUNDINGS',
...
export default (state: FundingState = initialState, action): FundingState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_FUNDINGS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_FUNDINGS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_FUNDINGS):
...
const apiSearchUrl = 'api/_search/fundings';

// Actions

export const getSearchEntities: ICrudSearchAction<IFunding> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_FUNDINGS,
  payload: axios.get<IFunding>(`${apiSearchUrl}?query=${query}`),
});
